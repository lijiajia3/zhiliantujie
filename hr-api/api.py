...
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "http://113.46.143.235",       # 正式伺服器 IP
        "http://localhost:3000",       # 本地開發
        "http://127.0.0.1:3000"        # 本地開發（某些瀏覽器）
    ],
...
def generate_resume_id(source_flag="0"):
    ...
    # Future: consider querying MongoDB for highest existing index
    max_index = 0
...
async def delete_employee_by_id(employee_id: str):
    # Attempt to match by MongoDB _id first
    filter_criteria = {"$or": []}
    try:
        filter_criteria["$or"].append({"_id": ObjectId(employee_id)})
    except Exception:
        pass
    # Also match by custom id or 工号
