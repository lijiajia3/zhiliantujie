          ...
          <button
            onClick={async () => {
              ...
              try {
                ...
                try {
                  const res = await axios.get(`/get-resume/${result.resume_id}`);
                  ...
                  if (err.response && err.response.status === 404) {
                    if (result.resume_id.endsWith("-1")) {
                      const formData = new FormData();
                      formData.append('file', file);
                      const analysisRes = await axios.post("/analyze-resume", formData, {
                        headers: { 'Content-Type': 'multipart/form-data' }
                      });
                      finalResult = {
                      ...
                      const analysisRes = await axios.post("/refresh-resume", { resume_id: result.resume_id });
                      finalResult = analysisRes.data;
                    }

                    await axios.post("/recommend-employee", {
